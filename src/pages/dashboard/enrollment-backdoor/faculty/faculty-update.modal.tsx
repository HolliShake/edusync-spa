...
export default function FacultyUpdateModal({
  state,
  onUpdated,
}: FacultyUpdateModalProps): ReactNode {
  ...
  const normalizeCampusCode = (campusShortName: string) => {
    const normalized = campusShortName
      .replaceAll(' ', '')
      .replaceAll('-', '')
      .replaceAll('_', '')
      .toLowerCase();
  ...
  useEffect(() => {
    ...
    const initialCampusId = Number(
      state.data.cycle?.campusId ?? state.data.academicProgram?.college?.campusId ?? 0
    );
  ...
  const onSubmit = async (values: UpdateEnrollmentValues) => {
    ...
    const payload: EnrollmentBackdoorDto = {
      ...
      campusCode: normalizeCampusCode(
        campusOptions.find((item) => item.id === values.campusId)?.shortName ?? record.campusCode
      ),
    };
    ...
    try {
      ...
      let updatedRecord: EnrollmentBackdoorDto = {
        ...payload,
        academicProgram:
          values.academicProgramId === record.academicProgramId
      ...
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        <div className="space-y-4">
          <div className="space-y-1">
              ...
              render={({ field }) => (
                ...
                >
                  <SelectTrigger id="edit-campus" className="w-full">
                    <SelectValue
                      placeholder={isLoadingCampuses ? 'Loading campuses...' : 'Select campus'}
                    />
                  </SelectTrigger>
                  <SelectContent>
            ...
            />
            {errors.campusId && (
              <p className="text-sm text-destructive">{errors.campusId.message}</p>
            )}
          </div>
              ...
              render={({ field }) => (
                ...
                >
                  <SelectTrigger id="edit-college" className="w-full">
                    <SelectValue
                      placeholder={isLoadingColleges ? 'Loading colleges...' : 'Select college'}
                    />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <div className="space-y-1">
              ...
              render={({ field }) => (
                ...
                >
                  <SelectTrigger id="edit-program" className="w-full">
                    <SelectValue
                      placeholder={isLoadingPrograms ? 'Loading programs...' : 'Select program'}
                    />
                  </SelectTrigger>
                  <SelectContent>
            ...
            {errors.academicProgramId && (
              <p className="text-sm text-destructive">{errors.academicProgramId.message}</p>
            )}
          </div>
        </div>
              ...
              render={({ field }) => (
                ...
                >
                  <SelectTrigger id="edit-cycle" className="w-full">
                    <SelectValue
                      placeholder={isLoadingCycles ? 'Loading cycles...' : 'Select cycle'}
                    />
                  </SelectTrigger>
                  <SelectContent>
        ...
        <div className="space-y-1">
          ...
          />
          {errors.sectionName && (
            <p className="text-sm text-destructive">{errors.sectionName.message}</p>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            ...
            />
            {errors.yearLevel && (
              <p className="text-sm text-destructive">{errors.yearLevel.message}</p>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            />
            {errors.termNumber && (
              <p className="text-sm text-destructive">{errors.termNumber.message}</p>
            )}
          </div>
          ...
          <div className="space-y-1">
            ...
            <Input
              id="edit-enrollment-role"
              className="w-full"
              type="number"
              min={1}
              {...register('enrollmentRoleId')}
              disabled={isSubmitting}
            />
            {errors.enrollmentRoleId && (
              <p className="text-sm text-destructive">{errors.enrollmentRoleId.message}</p>
            )}
          </div>
        </div>
