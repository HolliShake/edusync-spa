      ...
      render: (_, row) => (
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-xs text-muted-foreground">
            {row.course?.courseDescription || 'No description'}
          </span>
          ...
          <span className="text-xs text-muted-foreground">
            {row.course?.withLaboratory ? 'With Laboratory' : 'No Laboratory'}
          </span>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (_, row) => (
        <Badge variant="secondary">
          {row.enrollmentRole?.enrollmentRoleName ??
            row.enrollmentRole?.roleName ??
            'Student/Learner'}
        </Badge>
      ),
    },
...
function AddStudentModal({
  ...
  const [usersResponse, setUsersResponse] =
    useState<GetPaginatedResponseDto<PaginatedUser>>(EMPTY_USERS);
  ...
  const createNewStudent = async (userId: string) => {
    ...
    try {
      let normalizedCampusCode = (
        state.data?.academicProgram?.college?.campus?.campusShortName ?? ''
      )
        .replaceAll(' ', '')
        .replaceAll('-', '')
        .replaceAll('_', '')
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            <span>Rows</span>
            <Select
              value={String(userRows)}
              onValueChange={(value) => {
                setUserRows(Number(value));
                setUserPage(1);
              }}
            >
          ...
          <Button
            ...
            onClick={() =>
              setUserPage((previousPage) =>
                Math.min(usersResponse.paginationMeta.totalPages, previousPage + 1)
              )
            }
            disabled={
              userPage >= usersResponse.paginationMeta.totalPages || isLoadingUsers || isSubmitting
            }
          >
            Next
          </Button>
        ...
        <div className="flex justify-end gap-2">
          <Button
            ...
            disabled={isSubmitting || selectedUserId?.length <= 0}
          >
            Cancel
          </Button>
          <Button
...
export default function FacultySectionsPage(): React.ReactNode {
  ...
  const [studentsResponse, setStudentsResponse] =
    useState<GetPaginatedResponseDto<EnrollmentBackdoorDto>>(EMPTY_STUDENTS);
  ...
  return (
    ...
    >
      <div className="grid gap-3 rounded-md border p-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <div>
          ...
          <p className="text-sm font-medium">
            {selectedFaculty.academicProgram?.college?.campus?.campusShortName ?? '—'}
          </p>
        </div>
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {selectedFaculty.course?.courseTitle ?? 'No title'}
          </p>
          <p className="text-xs text-muted-foreground">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button variant="default" onClick={() => addStudentModalState.openFn(selectedFaculty)}>
            Enroll Student
          </Button>
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <p>
          ...
          <span className="font-semibold text-foreground">
            {studentsResponse.paginationMeta.totalItems}
          </span>{' '}
          student(s)
        </p>
      </div>
      ...
      <AddStudentModal
        state={addStudentModalState}
        onCreated={() => setRefetchKey((previousValue) => previousValue + 1)}
      />
        ...
        description={(data) => {
          ...
          return (
            <>
              ...
              <span className="font-semibold text-foreground">{name}</span> from this section? This
