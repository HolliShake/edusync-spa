...
export const decodeAccessList = (accessListStrArray: string): AccessActionPair[] => {
  try {
    ...
    return accessList.map((accessStr) => {
      ...
      const accessKey: AccessKey | undefined = Object.keys(AccessMapping).find(
        (key) => AccessMapping[key as AccessKey] === accessCode
      ) as AccessKey;
      const actionKey: ActionKey | undefined = Object.keys(ActionMapping).find(
        (key) => ActionMapping[key as ActionKey] === actionCode
      ) as ActionKey;
