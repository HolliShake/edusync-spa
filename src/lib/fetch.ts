...
const BACKDOOR_BASE =
  import.meta.env.VITE_APP_API_BACKDOOR_URL || 'https://cqi.ustp.edu.ph/dev/Api';
...
const routeOrvalRequest = async (meth: MethodType, url: string, options: RequestInit = {}) => {
  ...
  if (meth === 'PUT' && path.startsWith('Agency/update/') && body instanceof FormData) {
    return toJsonResponse(
      await api.updateAgency(Number(path.split('/').at(-1)), formDataToRecord(body) as never)
    );
  }
  ...
  if (meth === 'PUT' && path.startsWith('Campus/update/') && body instanceof FormData) {
    return toJsonResponse(
      await api.updateCampus(Number(path.split('/').at(-1)), formDataToRecord(body) as never)
    );
  }
